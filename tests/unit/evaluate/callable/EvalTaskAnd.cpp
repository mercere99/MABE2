...
TEST_CASE("EvalTaskAnd", "[evaluate/callable]"){
  ...
  uint32_t three = 3;
  uint32_t five = 5;
  CHECK(task.CheckTwoArg(zero, zero, one));
  CHECK(task.CheckTwoArg(zero, two, one));
  CHECK(task.CheckTwoArg(one, three, one));
  CHECK(task.CheckTwoArg(one, five, one));
  CHECK(task.CheckTwoArg(one, five, three));
