...
namespace mabe {
  ...
  protected:
    ...
    struct BaseTrait {
      ...
      BaseTrait(const std::string & _n, const std::string & _d, size_t _c=1)
        : name(_n), desc(_d), count(_c), config_name(name + "_trait") { }
      virtual ~BaseTrait() { }
    ...
    template <typename T> struct BaseTrait_Default : public BaseTrait {
      T default_value{};

      BaseTrait_Default(const std::string & _n, const std::string & _d, size_t _c)
        : BaseTrait(_n,_d,_c) { }
    ...
    template <typename T> struct PrivateTrait : public BaseTrait_Default<T> {
      PrivateTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddPrivateTrait<T>(BaseTrait::name, BaseTrait::desc, base_t::default_value, base_t::count); }
    };

    // Traits that are world readable, but write-protected
    template <typename T> struct OwnedTrait : public BaseTrait_Default<T> {
      OwnedTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddOwnedTrait<T>(BaseTrait::name, BaseTrait::desc, base_t::default_value, base_t::count); }
    };

    // Traits that are write-protected but MUST be read by another.
    template <typename T> struct GeneratedTrait : public BaseTrait_Default<T> {
      GeneratedTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddGeneratedTrait<T>(BaseTrait::name, BaseTrait::desc, base_t::default_value, base_t::count); }
    };

    // Traits that can be freely shared (read or write) with others.
    template <typename T> struct SharedTrait : public BaseTrait_Default<T> {
      SharedTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddSharedTrait<T>(BaseTrait::name, BaseTrait::desc, base_t::default_value, base_t::count); }
    };

    // Traits that must be generated by another.
    template <typename T> struct RequiredTrait : public BaseTrait_Default<T> {
      RequiredTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddRequiredTrait<T>(BaseTrait::name, base_t::count); }
    };

    // Traits that will be used if created by another, but are not required.
    template <typename T> struct OptionalTrait : public BaseTrait_Default<T> {
      OptionalTrait(const std::string & name, const std::string & desc, size_t count=1)
        : BaseTrait_Default<T>(name, desc, count) { }
      using base_t = BaseTrait_Default<T>;
      void AddTrait(Module & mod) { mod.AddOptionalTrait<T>(BaseTrait::name, BaseTrait::desc, base_t::count); }
    };
  ...
  public:
    ...
    TraitInfo & AddOptionalTrait(const std::string & name, const std::string & desc, size_t count=1) {
      return AddTrait<T,ALT_Ts...>(TraitInfo::Access::OPTIONAL, name, desc, T(), count);
    }
