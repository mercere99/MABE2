 ...
 *  @date 2019-2021.
 *
 *  @file  VirtualCPU_Inst_Input.hpp
 *  @brief Provides VirtualCPUOrgs an IO function that loads a new input.
 * 
 */

#ifndef MABE_VIRTUAL_CPU_INST_INPUT_H
#define MABE_VIRTUAL_CPU_INST_INPUT_H
...
namespace mabe {
  ...
  class VirtualCPU_Inst_Input : public Module {
  ...
  public:
    VirtualCPU_Inst_Input(mabe::MABE & control,
                    const std::string & name="VirtualCPU_Inst_Input",
                    const std::string & desc="Input instructions for VirtualCPUOrg population")
      : Module(control, name, desc), 
        target_collect(control.GetPopulation(1),control.GetPopulation(0)){;}
    ~VirtualCPU_Inst_Input() { }
    ...
    void SetupFuncs(){
      ...
      action_map.AddFunc<void, VirtualCPUOrg&, const VirtualCPUOrg::inst_t&>("IO", func_input);
    }
  ...
  MABE_REGISTER_MODULE(VirtualCPU_Inst_Input, "Input instructions for VirtualCPUOrg");
}
